...
if [[ "$CW_MODE" == "wrapcont" ]];then
    _CONTAINER_EXEC="$(which singularity) --silent exec  _deploy/$CW_CONTAINER_IMAGE"
    _RUN_CMD="$(which singularity) --silent exec \$DIR/../\$CONTAINER_IMAGE"
    _SHELL_CMD="$(which singularity) --silent shell \$DIR/../\$CONTAINER_IMAGE"
else
    _CONTAINER_EXEC="$(which singularity) --silent exec -B _deploy/$CW_SQFS_IMAGE:$CW_INSTALLATION_PATH:image-src=/ _deploy/$CW_CONTAINER_IMAGE"
